...
올바른 IBM ID로 [{{site.data.keyword.slportal}}](https://control.softlayer.com)에서 로그인했지만 {{site.data.keyword.slportal}}에서 IBM ID 인증으로 전환되지 않았습니다.
{: tsCauses}
...
IBM ID를 사용하여 로그인하면 로그인 성공 메시지가 표시되지만, [{{site.data.keyword.Bluemix_notm}} 콘솔](https://console.{DomainName}) 또는 [{{site.data.keyword.slportal}}](https://control.softlayer.com)로 돌아가지 않습니다.
{: tsSymptoms}
