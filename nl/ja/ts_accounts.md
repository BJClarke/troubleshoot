...
新規 {{site.data.keyword.Bluemix_notm}} 地域を作成しようとするとエラー・メッセージを受け取ります。{: tsSymptoms}

これは、1 つのパブリック地域のみでの開発をサポートする、標準アカウントを使用していることが原因と考えられます。アカウントを最初にセットアップするときに、作業を行う {{site.data.keyword.Bluemix_notm}} パブリック地域を選択します。
{: tsCauses}

標準アカウントを使用している場合は、追加の地域にアクセスするために、有料アカウントにアップグレードできます。コンソールで**「管理」>「請求および使用量」>「請求処理」**ページに移動し、**「クレジット・カードの追加」**をクリックします。**「請求処理」**ページでは、標準アカウントを使用しているかどうかを確認することもできます。
{: tsResolve}

## 新規組織を作成できない
{: #nosecondorg}
 
新規組織を作成しようとするとエラー・メッセージを受け取ります。
{: tsSymptoms}

これは、1 つの組織のみでの開発をサポートする、標準アカウントを使用していることが原因と考えられます。アカウントを最初にセットアップするときに、組織を作成します。
{: tsCauses}

標準アカウントを使用している場合は、追加の組織にアクセスするために、有料アカウントにアップグレードできます。コンソールで**「管理」>「請求および使用量」>「請求処理」**ページに移動し、**「クレジット・カードの追加」**をクリックします。**「請求処理」**ページでは、標準アカウントを使用しているかどうかを確認することもできます。
{: tsResolve}

## 新規のライト・プラン・インスタンスを作成できない
{: #nosecondlite}

新規のライト・プラン・インスタンスを作成しようとすると、次のエラー・メッセージが表示されます。
{: tsSymptoms}
...
標準アカウントからのアップグレードを希望せず、かつ既存のライト・サービス・インスタンスをもう使用していない場合は、既存のライト・プラン・インスタンスを「サービス」ダッシュボードから削除してから、新規インスタンスを作成できます。 
...
標準アカウントでは、Cloud Foundry アプリは最大 256 MB の瞬間的ランタイム・メモリーを使用できます。有料アカウントには、2 GB のメモリー制限があります。
{: tsCauses}

標準アカウントを使用している場合は、有料アカウントにアップグレードすることで、追加メモリーを取得できます。コンソールで**「管理」>「請求および使用量」>「請求処理」**ページに移動し、**「クレジット・カードの追加」**をクリックします。
{: tsResolve}

標準アカウントからのアップグレードは希望しないが、アイドル状態のアプリがいくつかある場合は、アイドル状態のアプリを削除して、一部のランタイム・メモリーを解放できます。 
