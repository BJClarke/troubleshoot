...
这可能是因为您使用的是 Lite 帐户，该帐户仅支持在一个公共区域中进行开发。您选择在首次设置帐户时要在其中工作的 {{site.data.keyword.Bluemix_notm}} 公共区域。
{: tsCauses}

如果您拥有 Lite 帐户，那么可以升级到计费帐户以访问其他区域。请转至控制台中的**管理 > 计费和使用情况 > 计费**页面，然后单击**添加信用卡**。在**计费**页面上，还可以检查您是否拥有 Lite 帐户。
{: tsResolve}
...
这可能是因为您使用的是 Lite 帐户，该帐户仅支持在一个组织中进行开发。您在首次设置帐户时创建组织。
{: tsCauses}

如果您拥有 Lite 帐户，那么可以升级到计费帐户以访问其他组织。请转至控制台中的**管理 > 计费和使用情况 > 计费**页面，然后单击**添加信用卡**。在**计费**页面上，还可以检查您是否拥有 Lite 帐户。
{: tsResolve}

## 无法创建新 Lite 套餐实例
{: #nosecondlite}

尝试创建新 Lite 套餐实例时收到以下错误消息：
{: tsSymptoms}
...
如果您不想从 Lite 帐户升级且不再使用现有的 Lite 服务实例，那么可以从仪表板中删除现有 Lite 套餐实例，然后创建新实例。
...
在 Lite 帐户中，Cloud Foundry 应用程序最多可以使用 256 MB 的即时运行时内存。在计费帐户中，存在 2 GB 的内存限制。
{: tsCauses}

如果使用的是 Lite 帐户，那么可以升级到计费帐户以获取其他内存。请转至控制台中的**管理 > 计费和使用情况 > 计费**页面，然后单击**添加信用卡**。
{: tsResolve}

如果您不想从 Lite 帐户升级，但拥有一些空闲的应用程序，那么可以删除空闲的应用程序以释放一些运行时内存。
